...
typedef struct _UAV_
{
	...
	AStar::CoordinateList pathNext;
	int     AttackUavNo = -1;
	int     totalValue = 0;//飞机价值和它的物品价值的总和
	int     uavValue = 0;
	int     goodsValue = 0;
	int     remain_electricity;
	bool    IsFull = false;//电量是否充满，不满就不起飞
	bool    haveDeal = false;
	GOODS   targetGood;
}UAV;
...
typedef struct _REGION_
{
	...
	int goodsNum;
	float WePakDistance;
	float EnePakDistance;
}REGION;
typedef struct _COST_PERF_
{
	int    uavIndex;
	int    goodsIndex;
	float   costPerformence;
}COST_PERF;
