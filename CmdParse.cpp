...
int ParserUav(cJSON *pUavArray, UAV *astUav, int *pNum)
{
    ...
    *pNum = cJSON_GetArraySize(pUavArray);
    ...
    for (int i = 0; i < *pNum; i++)
    {
		...
		nRet = JSONGetValue(pUAV, "remain_electricity", false, &(astUav[i].remain_electricity));
		if (nRet != 0)
		{
			printf("JSONGetValue error\n");
			return nRet;
...
{
    int         nRet = 0;
    cJSON       *pRoot = cJSON_CreateObject();//创建根数据对象 
    ...
    for (int i = 0; i < pstPlane->nUavNum; i++)
    {
		...
		cJSON_AddItemToObject(pUav, "remain_electricity", cJSON_CreateNumber(pstPlane->astUav[i].remain_electricity));
